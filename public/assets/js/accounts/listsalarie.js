$(document).ready(function () {
  $(document).on('click', '#togglePasswordButton', function(event) {
    ...
    var usersTablesal = $("#usersTablesal").DataTable({
        ajax: {
             ...
             var badgeClass = data === true ? "badge-success" : "badge-danger";
...

              {
                ...
                render: function (data, type, row) {
                  ...
                  var email = row.email;
                  var deactivateButton = '<button class="btn btn-outline-danger deactivatesal" data-username="' +
                    username +
                    '">Désactiver</button>';
                  var activateButton = '<button class="btn btn-outline-success activatesal" style="width:95px" data-username="' +
                    username +
                    '">Activer</button>';
          
                  return '<div class="btn-group" role="group">' +
                    '<button class="btn btn-outline-info editsal" data-username="' + username + '">Éditer</button>' +
                    '&nbsp;' +
                    '&nbsp;' +
    ...
    usersTablesal.column(1).search("Normal_user", true, false).draw();

    function showadd() {
        toastr.success('Responsable ajouté avec succés')
    }

    $("#openModalBtnsal").on("click", function () {
...
function submitForm(event) {
    event.preventDefault(); // Psalent the default form submission
  
    // Get the form data
    const form = document.getElementById('userFormsal');
  ...
  const submitButton = document.querySelector('#userFormsal button[type="submit"]');
  ...
  $(document).ready(function() {
    $(document).on("click", ".editsal", function (e) {
        ...
        fetch(WEBSERVERD_URL +'api/user/users_infos/?username=' + username, {
          ...
          .then(data => {
            ...
            if (user) {
              ...
              document.getElementById('edit_seniority').value = user.seniority;
      
              // Open the modal
              $('#editModalsal').modal('show');
            } else {
      ...
      $(document).ready(function() {
        ...
        var editForm = document.getElementById('editFormsal');
        editForm.addEventListener('submit', function(event) {
          ...
          fetch(WEBSERVERD_URL +'api/user/' + username, {
            ...
            .then(data => {
              ...
              usersTablesal.ajax.reload();
              $('#editModalsal').modal('hide');
            })
            .catch(error => {
              console.log('Error:', error);
            });
...
$('#usersTablesal').on('click', '.deactivatesal', function () {
    var username = $(this).data('username');
  
    $('#deactivateModalLabelsal').text("Confirmation de désactivation - " + username);
    $('#confirmDeactivateBtnsal').data('username', username);
    $('#deactivateModalsal').modal('show');
  });
  
  // Gestionnaire d'événement pour le clic sur le bouton de confirmation de désactivation
  $('#confirmDeactivateBtnsal').on('click', function () {
    var username = $(this).data('username');
    $('#deactivateModalsal').modal('hide');
  ...
  $("#retourButtonsald").on("click", function () {
    $("#deactivateModalsal").modal("hide");
  });
  // Gestionnaire d'événement pour le clic sur le bouton d'activation
  $('#usersTablesal').on('click', '.activatesal', function () {
    var username = $(this).data('username');
  
    $('#activateModalLabelsal').text("Confirmation d'activation - " + username);
    $('#confirmActivateBtnsal').data('username', username);
    $('#activateModalsal').modal('show');
  });
  
  // Gestionnaire d'événement pour le clic sur le bouton de confirmation d'activation
  $('#confirmActivateBtnsal').on('click', function () {
    var username = $(this).data('username');
    $('#activateModalsal').modal('hide');
  ...
  $("#retourButtonsala").on("click", function () {
    $("#activateModalsal").modal("hide");
  });
